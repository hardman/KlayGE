...
void VDMParticleApp::OnResize(uint32_t width, uint32_t height)
{
	...
	RenderViewPtr half_res_max_ds_view = rf.Make2DDepthStencilRenderView(width / 2, height / 2, EF_D24S8, 1, 0);
	RenderViewPtr quarter_res_max_ds_view = rf.Make2DDepthStencilRenderView(width / 4, height / 4, EF_D24S8, 1, 0);

	half_res_tex_ = rf.MakeTexture2D(width / 2, height / 2, 1, 1, fmt, 1, 0, EAH_GPU_Read | EAH_GPU_Write, nullptr);
	half_res_min_max_ds_tex_ = rf.MakeTexture2D(width / 2, height / 2, 1, 1, EF_GR16, 1, 0, EAH_GPU_Read | EAH_GPU_Write, nullptr);
	half_res_fb_->Attach(FrameBuffer::ATT_Color0, rf.Make2DRenderView(*half_res_tex_, 0, 1, 0));
	half_res_fb_->Attach(FrameBuffer::ATT_DepthStencil, half_res_max_ds_view);

	quarter_res_tex_ = rf.MakeTexture2D(width / 4, height / 4, 1, 1, fmt, 1, 0, EAH_GPU_Read | EAH_GPU_Write, nullptr);
	quarter_res_min_max_ds_tex_ = rf.MakeTexture2D(width / 4, height / 4, 1, 1, EF_GR16, 1, 0, EAH_GPU_Read | EAH_GPU_Write, nullptr);
	quarter_res_fb_->Attach(FrameBuffer::ATT_Color0, rf.Make2DRenderView(*quarter_res_tex_, 0, 1, 0));
	quarter_res_fb_->Attach(FrameBuffer::ATT_DepthStencil, quarter_res_max_ds_view);

	vdm_color_tex_ = rf.MakeTexture2D(width / 4, height / 4, 1, 1, fmt, 1, 0, EAH_GPU_Read | EAH_GPU_Write, nullptr);
	...
	vdm_quarter_res_fb_->Attach(FrameBuffer::ATT_Color0, rf.Make2DRenderView(*vdm_color_tex_, 0, 1, 0));
	...
	vdm_quarter_res_fb_->Attach(FrameBuffer::ATT_DepthStencil, quarter_res_max_ds_view);

	copy_pp_->InputPin(0, scene_tex_);

	vdm_composition_pp_->InputPin(0, vdm_color_tex_);
...
uint32_t VDMParticleApp::DoUpdate(uint32_t pass)
{
	...
	case 1:
		{
			...
			{
				uint32_t w = scene_ds_tex_->Width(0);
				uint32_t h = scene_ds_tex_->Height(0);
				...
				depth_to_min_max_pp_->InputPin(0, scene_ds_tex_);
				depth_to_min_max_pp_->OutputPin(0, half_res_min_max_ds_tex_);
				depth_to_min_max_pp_->Apply();

				if (PRT_NaiveHalfRes == particle_rendering_type_)
				{
					copy_to_depth_pp_->SetParam(0, 1);
					copy_to_depth_pp_->OutputPin(0, half_res_tex_);
					copy_to_depth_pp_->OutputFrameBuffer()->Attach(FrameBuffer::ATT_DepthStencil,
						half_res_fb_->Attached(FrameBuffer::ATT_DepthStencil));
					copy_to_depth_pp_->InputPin(0, half_res_min_max_ds_tex_);
					copy_to_depth_pp_->Apply();
				}
				else
				{
					w = half_res_min_max_ds_tex_->Width(0);
					h = half_res_min_max_ds_tex_->Height(0);
					reduce_min_max_pp_->SetParam(0, float2(0.5f / w, 0.5f / h));
					reduce_min_max_pp_->SetParam(1, float2(static_cast<float>((w + 1) & ~1) / w,
						static_cast<float>((h + 1) & ~1) / h));
					reduce_min_max_pp_->InputPin(0, half_res_min_max_ds_tex_);
					reduce_min_max_pp_->OutputPin(0, quarter_res_min_max_ds_tex_);
					reduce_min_max_pp_->Apply();

					copy_to_depth_pp_->SetParam(0, 1);
					copy_to_depth_pp_->OutputPin(0, quarter_res_tex_);
					copy_to_depth_pp_->OutputFrameBuffer()->Attach(FrameBuffer::ATT_DepthStencil,
						quarter_res_fb_->Attached(FrameBuffer::ATT_DepthStencil));
					copy_to_depth_pp_->InputPin(0, quarter_res_min_max_ds_tex_);
					copy_to_depth_pp_->Apply();
				}
	...
	default:
		...
		{
		case PRT_NaiveHalfRes:
			add_copy_pp_->InputPin(0, half_res_tex_);
			add_copy_pp_->Apply();
			break;

		case PRT_NaiveQuarterRes:
			add_copy_pp_->InputPin(0, quarter_res_tex_);
			add_copy_pp_->Apply();
			break;
